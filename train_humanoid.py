...
"""
Script for training the Humanoid.
"""


def train(args):
    """
    Train a humanoid agent with the specified training parameters and curriculum.
    ...
    if 'stdev' not in args.curriculum:
        ...
        env = make_vec_env(args.env_name, args.num_envs, env_kwargs={'assist_coeff': 1.0,
                                                                     ...
                                                                     'assist_anneal_rate': assist_anneal_rate,
                                                                     'ctrl_cost_weight': args.control_cost_weight})
    else:
        env = make_vec_env(args.env_name, args.num_envs, env_kwargs={'terminate_when_unhealthy': False,
                                                                     'ctrl_cost_weight': args.control_cost_weight})
...
def main():
    ...
    """
    parser = argparse.ArgumentParser('Train an agent with PPO in the customized Humanoid environment')
    parser.add_argument('--experiment-name', type=str, default='humanoid_debug', help='Name of the experiment')
    ...
    parser.add_argument('--hyperparam-file', type=str, default='parameters/humanoid_ppo_params.txt',
                        help='Path to hyperparameter file')
    parser.add_argument('--timesteps', type=int, default=2e5, help='Total training timesteps')
    parser.add_argument('--env-name', type=str, default='CustomHumanoid-v3', help='ID of the gym environment')
    parser.add_argument('--num-envs', type=int, default=1, help='Number of environments to run in parallel')
    parser.add_argument('--control-cost-weight', type=float, default=0.1, help='Weight of the control cost')
    ...
    parser.add_argument('--n-snapshots', type=int, default=0,
                        help='Number of policy snapshots to save during training. Only for "stdev" curriculum.')
    parser.add_argument('--curriculum', type=str, default='assist',
                        help='Which curriculum to use. Available options: "assist", "stdev"')
    parser.add_argument('--continuous-curr', action='store_true', default=False,
